...
def train(model, batch_size=1, clip=0.25, lr=20, log_interval=200, epoches=1):
            ...
            if batch % log_interval == 0 and batch > 0:
                ...
                print('| epoch {:3d} | {:5d}/{:5d} batches | lr {:02.2f} | ms/batch {:5.2f} | '
                      'loss {:5.2f} | ppl {:8.2f}'.format(
                          epoch, batch, len(trainSentences), lr,
...
model = SimpleLSTM(vocab_size=len(vocab), output_size=len(vocab),
                   embedding_dim=256, hidden_dim=256, n_layers=1)
device = torch.device('cpu')
model.to(device)
train(model, lr=0.1, epoches=5)
